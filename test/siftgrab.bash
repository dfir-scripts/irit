...
function show_menu(){
    ...
    NORMAL=`echo "\033[m"`
    RED=`echo "\033[31m"`
    echo -e "${GREEN} dfir-scripts 2025_11${NORMAL}" 
    echo -e "********************************************************"
    echo -e "${GRAY} Mount and Extract Information From Windows Disk Images${NORMAL}"
    echo -e "********************************************************"
    echo -e "**  1)${GREEN}  Process Windows Artifacts Using Available Parsers${NORMAL}"
    echo -e "**  2)${GREEN}  Extract Artifacts from Multiple System Acquisitions (e.g. KAPE, CyLR)${NORMAL}"
    echo -e "**  3) ${GREEN} Make a Backup Copy of Windows Artifacts from a Mounted Image(s)${NORMAL}"
    echo -e "**  4) ${GREEN} lf - Terminal File Manager${NORMAL}"
    echo -e "**  5) ${GREEN} Terminal${NORMAL}"
    echo -e "**  6) ${GREEN} Additional Parsers and Memory Analysis${NORMAL}"
    echo -e "**  7)${GREEN}  Process Windows Artifacts Using Zimmerman Tools${NORMAL}"
    echo ""
...
function get_computer_name(){
   ...
   [ "$comp_name" == "" ] && comp_name=$(date +'%Y-%m-%d-%H%M%S') && echo ""
...
function mount_vhd(){
      ...
      for ((i=1; i<=retries; i++)); do
          ermount -u
          mount_status=$(ermount -s | grep 'Active')
      ...
      sleep 2
      case_dir=$case0_dir
      echo "Attempting to mount $image_path..."
	  #********************
	  echo $d sleep 5
      modprobe nbd && echo "modprobe nbd"
      qemu-nbd -r -c /dev/nbd1 /"$d"
      ls /dev/nbd1  && make_green "nbd mount successful!"
      echo $d
      sleep 3
      #***********************
      for offset in 32256 8704 15872; do
          echo "Trying offset $offset..."
          mount -t ntfs -o ro,loop,show_sys_files,streams_interface=windows,offset=$offset /dev/nbd1 /mnt/image_mount 2>/dev/null
          echo mount -t ntfs -o ro,loop,show_sys_files,streams_interface=windows,offset=$offset /dev/nbd1 /mnt/image_mount
          sleep 3
          if [ $? -eq 0 ] && [ -d "/mnt/image_mount/C" ]; then
              ermount -s
              return 0
          fi
          echo "Offset $offset didn't work, trying next..."
          umount "$mount_point" >/dev/null 2>&1  # Clean up before next attempt
      done
        make_red "Mount Failed $d or does image does not contain a KAPE or CyLR image"
}
...
function mount_zip(){
    ...
    fuse-zip "$d" /mnt/image_mount
    # Check if mount was successful by verifying mount point has content
      target_dir=$(find /mnt/image_mount/ -maxdepth 2 -mindepth 2 -iname c)
      if [ ! -d $target_dir ]; then
        make_red "Mount Failed $d or does not contain a KAPE or CyLR image"
      fi
}
...
function recurse_source(){
  ...
  select_parsers
  counter=0
  vhd_files=$(find "$rmsource_path" -maxdepth 2 -type f \( -iname "*.vhd" -o -iname "*.vhdx" \))
  zip_files=$(find "$rmsource_path" -maxdepth 2 -type f -iname "*.zip" |grep zip$)
  if [ "$vhd_files" != "" ]; then
    find "$rmsource_path" -maxdepth 2 -type f \( -iname "*.vhd" -o -iname "*.vhdx" \) |while read d;
    do
      mount_vhd "$d"
    ...
    find "$rmsource_path" -maxdepth 2 -type f -iname "*.zip" |grep zip$|while read d; do
      mount_zip "$d"
      find /mnt/image_mount -maxdepth 2 -type d -iname c |while read dir;
      do
        case_dir=$case0_dir
        mount_dir="$dir"
           rprocess "$mount_dir"
    ...
    find "$rmsource_path" -maxdepth 2 -type d -iname c |while read dir;
    do
      case_dir=$case0_dir
      mount_dir="$dir"
       rprocess "$mount_dir"
...
function shim_cache(){
  if [ -f "$system_hive" ]; then
    ...
    dotnet /opt/zimmermantools/net9/AppCompatCacheParser.dll --csvf shimcache_$comp_name.csv -f $system_hive --csv $case_dir/Application_Execution/Shimcache
  fi
}
...
function extract_amcache.hve(){
  ...
  if [ -f "$case_dir/Application_Execution/Amcache/RegR-Amcache-$comp_name.csv" ]; then
    cat "$case_dir/Application_Execution/Amcache/RegR-Amcache-$comp_name.csv" |sed -r 's/(.*)\(/\1- /'|sed -r 's/(.*)\)/\1/'| \
    awk -F' - ' -v a=$comp_name '{print $3","$2","a}' >> "$case_dir/All_Summary/Amcache-Hashes-$comp_name.csv" && \
    sed -i '1i Hash,Path,System'  "$case_dir/All_Summary/Amcache-Hashes-$comp_name.csv"
  fi
}
...
function secrets_dump(){
  if [ -f "$sam_hive" ]; then
    ...
    TEMP=$(mktemp -d) && \
    cp {$sam_hive,$system_hive,$security_hive} "$TEMP/" && \
    secretsdump.py -sam "$TEMP/SAM" -system "$TEMP/SYSTEM" -security "$TEMP/SECURITY" LOCAL \
    >> $case_dir/User_Account_Info/secrets_dump-Hashes-$comp_name.txt && \
    rm -rf "$TEMP" 
  fi
}
...
function winservices() {
  if [ -f "$system_hive" ]; then
    ...
    if [ -f "$case_dir/Persistence/Services/winservices.py-$comp_name.csv" ]; then
      ...
      grep -E '^[0-9]{4}' "$case_dir/Persistence/Services/winservices.py-$comp_name.csv" | \
      awk -F',' -v comp_name="$comp_name" '{
        timestamp = $1
        cmd = "date -d \"" timestamp "\" +\"%s\" 2>/dev/null"
        cmd | getline tlntime
        close(cmd)
        tlninfo = "||[Service Last Write]: " $2 ";" $3 ";" $5 ";" $7 ";" $8
        print tlntime "|Svc|" comp_name tlninfo
    }' >> "$tempfile"
    fi
...
function parse_windows_index(){
  ...
  do
    ...
    /opt/venv/bin/python /usr/local/src/dfir-scripts/WinEventLogs/jsonl_to_csv.py $d > $index_file
  done
}
...
function browser_extract() {
  #Use sqlite3 to extract browser artifacts
  /opt/app/dfir-scripts/shellscripts/browser_extract.sh  $user_dir $comp_name >> "$case_dir/Browser_Activity/Browser-Extract-$comp_name.csv"
  /opt/app/dfir-scripts/shellscripts/browser_extract.sh -t  $user_dir $comp_name >> $tempfile

  # Run Hindsight on Users Directory
  echo "Running Hindsight on $user_dir Directory"
  mkdir -p $case_dir/Browser_Activity/extra
  /opt/venv/bin/hindsight.py -i $user_dir -o $case_dir/Browser_Activity/Hindsight -l "$case_dir/Browser_Activity/extra/hindsight.log" --temp_dir $case_dir/Browser_Activity/extra
...
sigma_output="$case_dir/Sigma_Alerts/Hayabusa-$comp_name.jsonl"
all_fields_csv="$case_dir/Sigma_Alerts/Hayabusa-Alerts-All-Fields-byDate-$comp_name.csv"
bydate_csv="$case_dir/Sigma_Alerts/Hayabusa-Alerts-byDate-$comp_name.csv"

if [ -f "$sigma_output" ]; then
    make_green "Standby... Processing $comp_name alerts — All-Fields"
    > "$all_fields_csv"
    printf 'Timestamp,EventID,Level,Computer,RuleTitle,Channel,All_Field_Info\n' > "$all_fields_csv"
    jq -r 'if type == "string" then fromjson? else . end
           | select(.Level != "info" and .Level != null)
           | (.Timestamp | .[:-10] | sub("T"; " ")) as $timestamp
           | [$timestamp, .EventID, .Level, .Computer, .RuleTitle, .Channel, .AllFieldInfo]
           | map(
               if type == "string" then
                 gsub(","; ";")  # Replace commas with semicolons
               else
                 tostring
               end
             )
           | join(",")' "$sigma_output" >> "$all_fields_csv"

    make_green "Standby... Creating a summary of Sigma Alerts Count-by-Date"
    printf 'Count,Date,Level,EventID,Computer,RuleTitle,Channel\n' > "$bydate_csv"
    cat $sigma_output | \
    jq -r 'select(.Level != "info")|",\(.Timestamp |.[:10]),\(.EventID),\(.Level),\(.Computer),\(.RuleTitle),\(.Channel)"' |sed 's/ ,/,/g' |sort | uniq -c |sort -rn >> "$bydate_csv"
    head -n 100 "$bydate_csv"
fi
}
...
function parse_evtx_jsonl(){
  ...
  if  [ "$(ls -A $case_dir/WindowsEventLogs/evtx_dump_$comp_name/ 2>/dev/null)" ]; then
    ...
    /opt/venv/bin/python /opt/app/dfir-scripts/WinEventLogs/ipv4inevtx.py -e -d $case_dir/WindowsEventLogs/evtx_dump_$comp_name/ >> $case_dir/All_Summary/IPv4-Addresses-in-evtx-$comp_name.csv
    head -n 10 $case_dir/All_Summary/IPv4-Addresses-in-evtx-$comp_name.csv 2>/dev/null
  fi
...
function consolidate_timeline(){
  if [ -f "$tempfile" ]; then
    strings $tempfile | sort -rn |uniq >> $timeline_dir/TLN/Triage-Timeline-$comp_name.TLN
    strings $tempfile |awk -F'|' '{$1=strftime("%Y-%m-%d %H:%M:%S",$1)}{print $1","$2","$3","$4","$5}'|sort -rn | uniq| grep -va ",,,,"| \
...
function z_lecmd_jlecmd(){
    ...
    do
      ...
      do
        dotnet /opt/zimmermantools/net9/LECmd.dll --csvf LECmd_$user_name-Recent-$comp_name.csv -d "$d/AppData/Roaming/Microsoft" --csv $case_dir/File_Access/LNK/$user_name-$comp_name
        dotnet /opt/zimmermantools/net9/LECmd.dll --csvf LECmd_$user_name-Desktop-$comp_name.csv -d "$d/Desktop" --csv $case_dir/File_Access/LNK/$user_name-$comp_name
        dotnet /opt/zimmermantools/net9/LECmd.dll --csvf LECmd_$user_name-Downloads-$comp_name.csv -d "$d/Downloads" --csv $case_dir/File_Access/LNK/$user_name-$comp_name
        dotnet /opt/zimmermantools/net9/LECmd.dll --csvf LECmd_$user_name-Documents-$comp_name.csv -d "$d/Documents" --csv $case_dir/File_Access/LNK/$user_name-$comp_name
        dotnet /opt/zimmermantools/net9/JLECmd.dll --csvf JLECmd_$user_name-$comp_name.csv -d "$d/AppData/Roaming/Microsoft/Windows/Recent" --csv $case_dir/Application_Execution/Jumplist/$user_name-$comp_name
      done
    done
  find $all_users/Microsoft/Windows/ -maxdepth 0 -type d |while read d;
  do
    dotnet /opt/zimmermantools/net9/LECmd.dll --csvf LECmd_AllUsers-$comp_name.csv -d "$d" --csv $case_dir/File_Access/LNK/AllUsers-$comp_name
    dotnet /opt/zimmermantools/net9/JLECmd.dll --csvf JLECmd_AllUsers-$comp_name.csv -d "$d" --csv $case_dir/Application_Execution/Jumplist/AllUsers-$comp_name
  done
  dotnet /opt/zimmermantools/net9/LECmd.dll --jsonf LECmd_Public.csv -d "$user_dir/Public" --csv $case_dir/File_Access/LNK/Public-$comp_name
}
#mftparser
function z_mftecmd(){
  if [ -f "$mount_dir/\$MFT" ]; then
    cd $mount_dir
    [ -f "\$MFT" ] && \
    dotnet /opt/zimmermantools/net9/MFTECmd.dll --jsonf MFTECmd_$comp_name.json -f \$MFT --$out_format $case_dir/File_System/MFT
    #dotnet /opt/zimmermantools/net9/MFTECmd.dll -f \$MFT --dr --csv $case_dir/File_System/MFT/Resident_Files
  fi
  #Copy ADS and Unallocated files
  if [ -f "$case_dir/File_System/MFT/MFTECmd_$comp_name.csv" ]; then
    head -n1 $case_dir/File_System/MFT/MFTECmd_$comp_name.csv > $case_dir/File_System/AlternateDataStreams/AlternateDataStreams-$comp_name.csv && \
    cat $case_dir/File_System/MFT/MFTECmd_$comp_name.csv | awk -F, '$14 ~ /^(True)$/{print $0}' >> $case_dir/File_System/AlternateDataStreams/MFTECmd-AlternateDataStreams-$comp_name.csv
    cat $case_dir/File_System/AlternateDataStreams/MFTECmd-AlternateDataStreams-$comp_name.csv | grep -va "Created0x10.ADS" | \
    ...
    cat $case_dir/File_System/MFT/MFTECmd_$comp_name.csv | awk -F, '$3 ~ /^(False)$/{print $0}'>> $case_dir/File_System/Deleted_Files/MFTECmd-Unallocated-$comp_name.csv
  fi
}
...
function parse_usn(){
  ...
  if [ -f \$Extend/\$UsnJrnl*\$J ]; then
    ...
    dotnet /opt/zimmermantools/net9/MFTECmd.dll --jsonf USNJRNL_$comp_name.json -m \$MFT -f \$Extend/\$UsnJrnl*\$J --$out_format $case_dir/File_System/USNJRNL
...
function z_rbcmd(){
  if [ -d "$mount_dir/\$Recycle.Bin" ]; then
    cd $mount_dir
    dotnet /opt/zimmermantools/net9/RBCmd.dll --csvf RBCmd_$comp_name.json -d '$Recycle.Bin/' --csv $case_dir/File_System/Deleted_Files 2>/dev/null
  fi
}
#recentfilecacheparser
function z_recentfilecacheparser(){
recent_file_cache="$windir/AppCompat/Programs/RecentFileCache.bcf" 
dotnet /opt/zimmermantools/net9/RecentFileCacheParser.dll --jsonf RFCmd_$comp_name.json -f "$recent_file_cache" --$out_format $case_dir/RecentFileCache
}
...
function z_rla(){
  ...
  cd "$regdir"
  dotnet /opt/zimmermantools/net9/rla.dll --nop --ca false -d "$regdir" --out $case_dir/Registry_Files/$comp_name-config 2>/dev/null
}

#sbecmd  shellbags
function z_sbecmd() {
  fd -i -d 2 -t f "ntuser.dat$" "$user_dir" 2>/dev/null | while read ntuser_path; do
    user_name=$(dirname "$ntuser_path" | awk -F"/" '{print $(NF-1)}')
    
    # Check if UsrClass.dat exists at the given path
   if [ $(fd -d 1 -i -t f "UsrClass.dat$" "$user_dir/$user_name/AppData/Local/Microsoft/Windows" 2>/dev/null | wc -l) -gt 0 ]; then
      # Use echo instead of dirname to print the directory
      echo "$user_dir/$user_name/AppData/Local/Microsoft/Windows" | while read dir; do
        dotnet /opt/zimmermantools/net9/SBECmd.dll --jsonf "SBECmd_$user_name-$comp_name.json" -d "$dir" --json "$case_dir/File_Access/ShellBags/SBECmd-$user_name-$comp_name"
      done
    fi
  done
}

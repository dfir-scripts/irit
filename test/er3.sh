...
mount_image() {
    ...
    if [ -n "$ewf_mount" ] || [ -n "$splitraw_mount" ] || [ -n "$aff_mount" ]; then
        ...
        # Virtual disks (VMDK, VDI, QCOW2, etc.): don't use -r
        # These are working copies, not forensic evidence
        qemu_nbd_opts=""
        echo "Detected virtual disk image format..."
    fi
    ...
    if command -v pvscan >/dev/null 2>&1; then
        ...
        if [ -n "$vgs_found" ]; then
            ...
            # Warn user for virtual disks with LVM (less severe than raw images)
                ...
                print_success "User confirmed. Proceeding with LVM activation..."
                echo ""
            # Warn user about read-only mount ONLY if -r flag is set
            # This should only happen for virtual disks that were explicitly mounted read-only
            # (Virtual disks are now mounted writable by default, so this shouldn't trigger)
            elif [[ " $qemu_nbd_opts " =~ " -r " ]] && [ -z "$ewf_mount" ] && [ -z "$aff_mount" ] && [ -z "$splitraw_mount" ]; then
                echo ""
                echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

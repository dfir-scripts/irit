...
print_warning() { echo -e "${YELLOW}$1${NC}"; }  # FIX: was called but never defined
...
cleanup_lvm() {
    ...
    local mapper_devices
    mapper_devices=$(ls -1 /dev/mapper/ 2>/dev/null | grep -v '^control$' || true)
    ...
    if [ -n "$mapper_devices" ]; then
        ...
        for mapper_dev in $mapper_devices; do
            ...
            if lvs "$mapper_path" >/dev/null 2>&1; then
                local vg_name
                vg_name=$(lvs --noheadings -o vg_name "$mapper_path" 2>/dev/null | tr -d ' ')

                if [ -n "$vg_name" ]; then
                    local pv_devices
                    pv_devices=$(pvs --noheadings -o pv_name -S vg_name="$vg_name" 2>/dev/null | tr -d ' ')
    ...
    # Step 2: Deactivate all volume groups on this NBD device
    local vgs_on_nbd
    vgs_on_nbd=$(pvs --noheadings -o vg_name -S "pv_name=~^${nbd_device}" 2>/dev/null | tr -d ' ' | sort -u || true)
    ...
    if [ -n "$vgs_on_nbd" ]; then
        for vg in $vgs_on_nbd; do
            ...
            if vgchange -an "$vg" 2>/dev/null; then
                ...
                if [ "$verbose" = "true" ]; then
                    print_warning "Could not deactivate volume group: $vg"
                fi
            fi
        done
    ...
    # Step 3: Final check - force-remove any still-active mapper devices for this NBD
    local remaining_mappers
    remaining_mappers=$(ls -1 /dev/mapper/ 2>/dev/null | grep -v '^control$' || true)
    ...
    if [ -n "$remaining_mappers" ]; then
        for mapper_dev in $remaining_mappers; do
            ...
            if lvs "$mapper_path" >/dev/null 2>&1; then
                local pv_devices
                pv_devices=$(pvs --noheadings -o pv_name -S "lv_path=$mapper_path" 2>/dev/null | tr -d ' ')
    ...
    # Step 4: Remove any stale PV cache entries
...
check_lvm_active() {
    ...
    local mapper_count
    mapper_count=$(ls -1 /dev/mapper/ 2>/dev/null | grep -v '^control$' | wc -l)
...
# Function to create a temporary LVM filter to prevent auto-activation
create_lvm_filter() {
...
# Function to remove the temporary LVM filter
remove_lvm_filter() {
    local filter_file="/etc/lvm/lvm.conf.d/er2-temp-$$.conf"
...
# ---------------------------------------------------------------------------
# FIX: check_mount_status — restored proper if/else/fi structure
# (previously split in two by the LVM filter functions being inserted mid-body)
# ---------------------------------------------------------------------------
check_mount_status() {
    local mount_point="$1"
...
# ---------------------------------------------------------------------------
# Smart cleanup wrapper — called on error paths
# ---------------------------------------------------------------------------
cleanup_and_exit() {
    local nbd_device="${1:-/dev/nbd1}"
    local temp_dir="$2"
    ...
    # FIX: unmount before removing directories (previously rm-rf'd without unmounting first)
...
unmount_image() {
    ...
    local ewf_mount="$3"
    local aff_mount="$4"
    local splitraw_mount="$5"   # FIX: was not accepted as a parameter previously
    ...
    if command -v vgchange >/dev/null 2>&1; then
        local lvm_on_nbd
        lvm_on_nbd=$(lsblk -ln -o NAME,TYPE "$nbd_device" 2>/dev/null | awk '$2 == "lvm" {print $1}' || true)
        if [ -n "$lvm_on_nbd" ]; then
            echo "Found active LVM volumes, deactivating..."
            local active_vgs
            active_vgs=$(vgs --noheadings -o vg_name 2>/dev/null | tr -d ' ')
    ...
    local nbd_size
    nbd_size=$(cat /sys/block/nbd1/size 2>/dev/null || echo "0")
    if [ "$nbd_size" != "0" ]; then
        ...
        for attempt in 1 2 3; do
            if qemu-nbd -d "$nbd_device" >/dev/null 2>&1; then
                sleep 0.5
                local verify_size
                verify_size=$(cat /sys/block/nbd1/size 2>/dev/null || echo "0")
        ...
        if [ "$detach_success" = false ]; then
            ...
            if rmmod nbd 2>/dev/null; then
                ...
                # Still attempt FUSE cleanup below before returning
            fi
        fi
    else
    ...
    # Step 6: Unmount split RAW FUSE mount
    ...
    # FIX: Clean up temp directories AFTER unmounting (previously rm-rf'd without unmounting)
    [ -n "$temp_dir" ]       && rm -rf "$temp_dir" 2>/dev/null
    ...
    # FIX: Always remove LVM filter on unmount (was missing from unmount_image)
    remove_lvm_filter
...
mount_image() {
    ...
    # ALWAYS create LVM filter to prevent auto-activation
    create_lvm_filter
    ...
    if [ "$unmount_only" = "true" ]; then
        unmount_image "$mount_point" "$temp_dir" "/mnt/raw" "/mnt/aff" "/mnt/raw"
        return $?
    fi
    ...
    if [ -z "$image_path" ]; then
        ...
        echo "Usage: $SCRIPT_NAME -i <image> [-m mount/point] [-f filesystem] [-l] [-o offset] [-s] [-u]"
        echo ""
        echo "Required:"
        echo "  -i <image>         Disk image file or ISO"
        echo ""
        echo "Optional:"
        echo "  -f <filesystem>    Filesystem type (auto-detected if not specified)"
        echo "  -l                 Enable LVM support (scans and activates LVM volumes)"
    ...
    if [ -n "$filesystem" ]; then
        case "$filesystem" in
            ntfs|ext2|ext3|ext4|vfat|exfat|hfsplus|xfs|zfs|btrfs|ufs|reiserfs|f2fs|jfs|iso9660|udf)
                ;;
            *)
                print_error "Unsupported filesystem '$filesystem'."
                echo "Supported: ntfs, ext2, ext3, ext4, vfat, exfat, hfsplus, xfs, zfs, btrfs, ufs, reiserfs, f2fs, jfs, iso9660, udf"
                return 1
                ;;
        esac
    fi
    ...
    local nbd_check_size
    nbd_check_size=$(cat /sys/block/nbd1/size 2>/dev/null || echo "0")
    ...
    if [[ "$image_path_lower" =~ \.ova$ ]]; then
        ...
        if [ -z "$image_path" ]; then
            print_error "No disk image found in OVA archive. Check contents with 'tar -tvf $image_path'."
            rm -rf "$temp_dir" 2>/dev/null
            return 1
        fi
    fi
    ...
    if [[ "$image_path_lower" =~ \.ovf$ ]]; then
        ...
        if [ -z "$image_path" ]; then
            print_error "No disk image found for OVF. Check directory with 'ls -l $ovf_dir'."
            return 1
        fi
    fi
    ...
    if [[ "$image_path_lower" =~ \.(000|001)$ ]]; then
        ...
        if [[ "$file_type" =~ AFF|"Advanced Forensic Format" ]]; then
            ...
            local original_aff_basename
            original_aff_basename=$(basename "$image_path")
            ...
            original_basename=$(basename "$image_path")
    ...
    # ISO — mount directly without NBD
    ...
    if ! [[ "$image_path_lower" =~ \.(vhd|vhdx|vdi|qcow|qcow2|vmdk|dd|img|raw|iso)$ \
         || "$image_path" =~ /ewf1$ \
         || "$image_path" =~ /mnt/aff/ \
         || "$image_path" =~ /mnt/raw/ ]]; then
        echo ""
    ...
    local fs_checks=(
        "ntfs:ntfs-3g:sudo apt install ntfs-3g"
        "vfat:mount.vfat:sudo apt install dosfstools"
        "exfat:mount.exfat:sudo apt install exfatprogs"
        "hfsplus:mount.hfsplus:sudo apt install hfsprogs"
    )
    for entry in "${fs_checks[@]}"; do
        local fs cmd install_hint
        fs="${entry%%:*}"; entry="${entry#*:}"; cmd="${entry%%:*}"; install_hint="${entry#*:}"
        if [ "$filesystem" = "$fs" ] && ! command -v "$cmd" >/dev/null 2>&1; then
            print_error "$fs support not found. Install with '$install_hint'."
            cleanup_and_exit "$nbd_device" "$temp_dir" "$ewf_mount" "$aff_mount" "$splitraw_mount" false
            return 1
        fi
    done

    if [ "$filesystem" = "xfs" ] && ! command -v mount.xfs >/dev/null 2>&1 && ! grep -q xfs /proc/filesystems 2>/dev/null; then
        print_error "XFS support not found. Install with 'sudo apt install xfsprogs'."
        cleanup_and_exit "$nbd_device" "$temp_dir" "$ewf_mount" "$aff_mount" "$splitraw_mount" false
        return 1
    fi
    if [ "$filesystem" = "btrfs" ] && ! command -v btrfs >/dev/null 2>&1; then
        print_error "Btrfs support not found. Install with 'sudo apt install btrfs-progs'."
        cleanup_and_exit "$nbd_device" "$temp_dir" "$ewf_mount" "$aff_mount" "$splitraw_mount" false
        return 1
    fi
    if [ "$filesystem" = "zfs" ] && ! command -v zpool >/dev/null 2>&1; then
        print_error "ZFS support not found. Install with 'sudo apt install zfsutils-linux'."
        cleanup_and_exit "$nbd_device" "$temp_dir" "$ewf_mount" "$aff_mount" "$splitraw_mount" false
        return 1
    fi
    if [ "$filesystem" = "f2fs" ] && ! grep -q f2fs /proc/filesystems 2>/dev/null; then
        print_error "F2FS support not found. Install with 'sudo apt install f2fs-tools'."
        cleanup_and_exit "$nbd_device" "$temp_dir" "$ewf_mount" "$aff_mount" "$splitraw_mount" false
        return 1
    fi
    if [ "$filesystem" = "reiserfs" ] && ! grep -q reiserfs /proc/filesystems 2>/dev/null; then
        print_error "ReiserFS support not found. Install with 'sudo apt install reiserfsprogs'."
        cleanup_and_exit "$nbd_device" "$temp_dir" "$ewf_mount" "$aff_mount" "$splitraw_mount" false
        return 1
    fi
    ...
    # LVM scan and optional activation
    if command -v pvscan >/dev/null 2>&1; then
        ...
        local vgs_found
        vgs_found=$(vgs --noheadings -o vg_name --config 'devices { filter=[ "a|/dev/nbd.*|", "r|.*|" ] }' 2>/dev/null | tr -d ' ')
        ...
        if [ -n "$vgs_found" ]; then
            ...
            if [ "$lvm_mode" = false ]; then
                ...
                echo ""
                echo "To access LVM volumes, you need to use the -l flag."
                echo "The image will remain read-only (protected by qemu-nbd -r)."
                echo ""
            ...
            echo ""
            echo "ℹ️  INFO: Read-Only LVM Activation"
            echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
            echo ""
            echo "The image is mounted read-only using qemu-nbd -r."
            echo "LVM volumes will be scanned and activated for access."
            echo ""
            echo "FORENSIC INTEGRITY:"
            echo "  • The original image file WILL NOT be modified"
            echo "  • Cryptographic hash (MD5/SHA-1/SHA-256) will remain unchanged"
            echo "  • Chain of custody is maintained"
            echo ""
            echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
            echo ""
    ...
    # Build partition list (exclude LVM2_member — mount the LVs, not raw PVs)
    local all_parts
    all_parts=$(lsblk -ln -o NAME | grep "^nbd1p" || true)
    local partitions=""
    for part in $all_parts; do
        local part_type
        part_type=$(blkid -o value -s TYPE "/dev/$part" 2>/dev/null || echo "")
    ...
    if [ -z "$partitions" ]; then
        ...
        local device_fs
        device_fs=$(blkid -o value -s TYPE "$nbd_device" 2>/dev/null || true)
        if [ -n "$device_fs" ]; then
    ...
    local lvm_volumes
    lvm_volumes=$(lvs --noheadings -o lv_path 2>/dev/null | tr -d ' ' || true)

    if [ -z "$lvm_volumes" ]; then
        local lvm_from_lsblk
        lvm_from_lsblk=$(lsblk -ln -o NAME,TYPE "$nbd_device" 2>/dev/null | awk '$2 == "lvm" {print $1}' || true)
    ...
    # -----------------------------------------------------------------------
    # Partition selection and mounting
    # -----------------------------------------------------------------------
    if [ -n "$partitions" ]; then
        ...
        if [ "$part_count" -gt 1 ]; then
            ...
            for part in $partitions; do
                ...
                if [[ "$part_device" =~ ^/dev/mapper/ ]] || [[ "$part_device" =~ ^/dev/.*/.* ]]; then
                    local vg_info
                    vg_info=$(lvs --noheadings -o vg_name "$part_device" 2>/dev/null | tr -d ' ' || echo "")
                    if [ -n "$vg_info" ]; then
            ...
            if command -v blkid >/dev/null 2>&1; then
                ...
                if [ -n "$blkid_fstype" ]; then
                    ...
                    case "$blkid_fstype" in
                        ...
                        ext4|ext3|ext2) mount_fstype="$blkid_fstype" ;;
                        hfsplus) mount_fstype="hfsplus" ;;
                        xfs) mount_fstype="xfs" ;;
                        zfs_member) mount_fstype="zfs" ;;
                        btrfs) mount_fstype="btrfs" ;;
                        ufs) mount_fstype="ufs" ;;
                        reiserfs) mount_fstype="reiserfs" ;;
                        f2fs) mount_fstype="f2fs" ;;
                        jfs) mount_fstype="jfs" ;;
                        iso9660) mount_fstype="iso9660" ;;
                        udf) mount_fstype="udf" ;;
            ...
            # Handle already-mounted device
            local existing_mount
            existing_mount=$(mount | grep "^$selected_part " | awk '{print $3}')
                ...
                case "$mount_choice" in
                    ...
                    2)
                        ...
                        if umount "$selected_part" 2>/dev/null; then
                            ...
                            print_error "Failed to unmount $selected_part. It may be in use. Try: lsof | grep $existing_mount"
                            cleanup_and_exit "$nbd_device" "$temp_dir" "$ewf_mount" "$aff_mount" "$splitraw_mount" false
                            return 1
                        fi
                        ;;
            ...
            case "$mount_fstype" in
                ...
                ext2|ext3|ext4)
                    ...
                    ;;
                xfs)
                    mount_cmd="mount -t xfs -o $mount_mode,norecovery $selected_part $mount_point"
                    ;;
                btrfs)
                    mount_cmd="mount -t btrfs -o $mount_mode,nospace_cache $selected_part $mount_point"
                    ;;
                zfs)
                    echo "ZFS detected. Attempting zpool import in read-only mode..."
                    local zpool_name
                    zpool_name=$(zdb -l "$selected_part" 2>/dev/null | grep -m1 'name:' | awk '{print $2}' | tr -d "'")
                    if [ -n "$zpool_name" ]; then
                        zpool import -o readonly=on -R "$mount_point" "$zpool_name" 2>/dev/null || {
                            zpool import -f -o readonly=on -R "$mount_point" "$zpool_name" 2>/dev/null || {
                                print_error "Failed to import ZFS pool '$zpool_name'."
                                echo "Try manually: zpool import -f -o readonly=on -R $mount_point $zpool_name"
                                cleanup_and_exit "$nbd_device" "$temp_dir" "$ewf_mount" "$aff_mount" "$splitraw_mount" false
                                return 1
                            }
                        }
                        print_success "ZFS pool '$zpool_name' imported read-only at $mount_point"
                        echo ""
                        echo "ZFS datasets:"
                        zfs list -r "$zpool_name" 2>/dev/null || true
                        echo ""
                        return 0
                    else
                        print_error "Could not determine ZFS pool name from $selected_part."
                        echo "Try manually: zdb -l $selected_part"
                        cleanup_and_exit "$nbd_device" "$temp_dir" "$ewf_mount" "$aff_mount" "$splitraw_mount" false
                        return 1
                    fi
                    ;;
                reiserfs)
                    mount_cmd="mount -t reiserfs -o $mount_mode,nolog $selected_part $mount_point"
                    ;;
                jfs)
                    mount_cmd="mount -t jfs -o $mount_mode,nointegrity $selected_part $mount_point"
                    ;;
                f2fs)
                    mount_cmd="mount -t f2fs -o $mount_mode,disable_roll_forward $selected_part $mount_point"
                    ;;
                ufs)
                    mount_cmd="mount -t ufs -o $mount_mode,ufstype=ufs2 $selected_part $mount_point"
                    ;;
                iso9660)
                    mount_cmd="mount -t iso9660 -o $mount_mode $selected_part $mount_point"
                    ;;
                udf)
                    mount_cmd="mount -t udf -o $mount_mode $selected_part $mount_point"
                    ;;
                *)
            ...
            elif [[ "$mount_fstype" =~ ^ext[234]$ ]]; then
            ...
            if [ $mount_status -eq 0 ] && mountpoint -q "$mount_point"; then
                ...
                if [ -n "$dir_check" ] || [ -n "$(ls "$mount_point")" ]; then
                    ...
                    print_success "Success!!"
                    echo
                    # FIX: use $log_csv consistently (was mixed with $output_csv on guard check)
                    if [ -n "$log_csv" ]; then
                        local partition_size="Unknown"
                        if command -v fdisk >/dev/null 2>&1; then
                            partition_size=$(fdisk -l "$nbd_device" 2>/dev/null | grep "^$nbd_device" | head -n 1 | awk '{print $5}' | numfmt --to=iec-i --suffix=B --format="%.2f")
                        fi
                        echo "$mount_point,$partitions,0,$mount_fstype,\"$mount_cmd\",${partition_size:-Unknown},Success" >> "$log_csv"
                    fi
                    return 0
                fi
                umount "$mount_point" 2>/dev/null
            ...
            # Offset-based fallback for raw/unpartitioned images
            ...
            for offset_sectors in "${starting_sectors[@]}"; do
                ...
                if [ "$mount_fstype" = "ntfs" ]; then
                    ...
                    mount_cmd="mount -t $mount_fstype -o $mount_mode,noload,offset=$byte_offset $nbd_device $mount_point"
                else
                    mount_cmd="mount -t $mount_fstype -o $mount_mode,offset=$byte_offset $nbd_device $mount_point"
                fi
                ...
                if [ $mount_status -eq 0 ] && mountpoint -q "$mount_point"; then
                    ...
                    if [ -n "$dir_check" ] || [ -n "$(ls "$mount_point")" ]; then
                        ...
                        if [ -n "$log_csv" ]; then
                            ...
                            echo "$mount_point,$offset_sectors,$byte_offset,$mount_fstype,\"$mount_cmd\",${partition_size:-Unknown},Success" >> "$log_csv"
                        fi
                        return 0
                    fi
                fi
        ...
        print_error "No mountable partitions or volumes found in $image_path."
        echo "Try: fdisk -l $nbd_device  or  blkid $nbd_device"
        cleanup_and_exit "$nbd_device" "$temp_dir" "$ewf_mount" "$aff_mount" "$splitraw_mount" false
        return 1
    fi
}
...
if [[ "$1" == "-h" ]] || [[ "$1" == "--help" ]]; then
    ...
    echo "  Linux:             ext2, ext3, ext4, xfs, btrfs, f2fs, jfs, reiserfs"
    echo "  Windows:           ntfs, vfat, exfat"
    echo "  macOS:             hfsplus"
    echo "  BSD/Solaris:       ufs, zfs"
    echo "  Optical:           iso9660, udf"
    echo ""
    echo "Examples:"
...
while getopts "i:m:f:lo:su" opt; do
    case $opt in
        i) image_path="$OPTARG" ;;
...
mount_image "$image_path" "$mount_point" "$lvm_mode" "$filesystem" "$check_status" "$unmount" "" "$manual_offset"
exit $?
